<?php if ($adminAuthed): ?>
	<?php if ($adminEmail === ''): ?>
		<div class="subtitle-line notice-warn">No recovery email set. Without one, if you ever forget your password there is no way back in — you would be locked out permanently. Add your email in settings below. You can skip this for now and come back to it later.</div>
	<?php endif; ?>
	<form method="post" class="subtitle-form">
		...
		<input type="email" class="upload-auth-input" name="admin_email" maxlength="254" value="<?= htmlspecialchars($adminEmail, ENT_QUOTES, 'UTF-8') ?>" placeholder="Admin email (for password reset)">
		<button type="submit" name="save_settings" value="1" class="ui-btn">Save settings</button>
	</form>
	...
	<div class="hero-actions">
		<a href="?<?= $blogQ ?>download_backup=1" class="ui-btn">Download backup (.zip)</a>
	</div>

	<form method="post" class="upload-panel">
		<div class="subtitle-line">Change password</div>
		<input type="password" class="upload-auth-input" name="current_password" maxlength="120" placeholder="Current password" required>
		<input type="password" class="upload-auth-input" name="new_password" minlength="6" maxlength="120" placeholder="New password (min 6 chars)" required>
		<input type="password" class="upload-auth-input" name="confirm_password" maxlength="120" placeholder="Confirm new password" required>
		<div class="hero-actions">
			<button type="submit" name="change_password" value="1" class="ui-btn">Change password</button>
		</div>
	</form>
	...
	<form method="post" enctype="multipart/form-data" class="upload-panel" id="inlineUploadForm">
		...
		<?php endif; ?>
		<input type="file" id="inlineUploadFiles" class="upload-auth-input" name="files[]"
			accept="image/*,video/*,audio/*,.mp3,.wav,.flac" multiple required>
	...
	<form method="post" class="upload-panel" id="textPostForm">
		<textarea id="textPostContent" name="text_post_content" maxlength="<?= MAX_TEXT_POST_LENGTH ?>"
			placeholder="Write text post (max <?= MAX_TEXT_POST_LENGTH ?> chars)" required></textarea>
